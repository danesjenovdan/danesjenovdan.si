          ...
          if (this.honeyPotName !== '') {
            ...
            try {
              ...
              const checkoutResponse = await this.$axios.$get(
                `https://podpri.djnd.si/api/generic-donation/4/?question_id=1&answer=${encodeURIComponent(
                  this.answer,
                )}`,
              );
...
.checkout.checkout--pmvd /deep/ {
  ...
  .donation-option {
    ...
    &.donation-option--selected {
      .donation-option__content-wrapper {
        border-color: #160444;
        box-shadow: 0 0 0 0.2rem rgba(#160444, 0.25);
      }
      path.circle {
        fill: $white;
        stroke-width: 0;
  ...
  .confirm-button {
    ...
    border: 2px solid #160444;
    color: #160444;

    .hearts path:first-child {
      stroke: #160444;
    }
