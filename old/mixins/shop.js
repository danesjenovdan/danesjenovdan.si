  ...
  methods: {
    ...
    async getOrderKey() {
      ...
      if (!orderKey) {
        const newBasket = await this.$axios.$get(
          'https://podpri.lb.djnd.si/api/shop/basket/',
        );
    ...
    async getBasketItems(orderKey) {
      const basketItems = await this.$axios.$get(
        `https://podpri.lb.djnd.si/api/shop/items/?order_key=${orderKey}`,
      );
      return basketItems;
    },
    async addToBasket(orderKey, productId, amount = 1) {
      await this.$axios.$post(
        `https://podpri.lb.djnd.si/api/shop/add_to_basket/?order_key=${orderKey}`,
        {
    ...
    async changeAmount(orderKey, itemId, newAmount) {
      if (newAmount <= 0) {
        await this.$axios.$delete(
          `https://podpri.lb.djnd.si/api/shop/items/${itemId}/?order_key=${orderKey}`,
        );
      } else {
