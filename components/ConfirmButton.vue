<template>
  <dynamic-link
    ...
    :disabled="disabled || loading"
    @mouseenter="animateHearts"
    @mouseleave="stopAnimatingHearts"
    v-on="$listeners"
  >
    ...
    <div v-if="hearts" class="hearts">
      <div v-for="i in 3" :key="i" class="heart full">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="8 0 92 100"
          fill="#fff"
        >
          ...
          <path
            id="path12"
            :style="`fill:${heartFills[i - 1]};stroke-width:0.30070534`"
...
export default {
  ...
  data() {
    return {
      heartFills: ['transparent', 'transparent', 'transparent'],
      timer: null,
      heartIndex: 0,
    };
  },
  ...
  methods: {
    animateHearts() {
      this.timer = window.setInterval(() => {
        this.heartFills[this.heartIndex % 3] === 'transparent'
          ? this.$set(this.heartFills, this.heartIndex % 3, '#ffffff')
          : this.$set(this.heartFills, this.heartIndex % 3, 'transparent');
        this.heartIndex += 1;
      }, 100);
    },
    stopAnimatingHearts() {
