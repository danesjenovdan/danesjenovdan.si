<template>
  <div class="checkout">
    ...
    <checkout-stage v-if="stage === 'select-amount'" :stage="stage">
      <template slot="title">
        Izberi višino <strong>mesečne</strong> donacije!
      </template>
...
import CardPayment from '~/components/PaymentNew/Card.vue';
...
export default {
  nuxtI18n: {
    paths: {
      sl: '/doniraj/novi/:donationId',
      en: '/donate/new/:donationId',
    },
  },
  components: {
    ConfirmButton,
  ...
  mounted() {
    ...
    if (!this.$route.params.donationId) {
      // throw error
  ...
  methods: {
          ...
          if (this.honeyPotName !== '') {
            ...
            try {
              ...
              const checkoutResponse = await this.$axios.$get(
                `https://podpri.lb.djnd.si/api/generic-donation/${
                  this.donationId
    ...
    async paymentSuccess({ nonce } = {}) {
      ...
      this.nonce = nonce;
      const paymentURL = `https://podpri.lb.djnd.si/api/generic-donation/subscription/${this.donationId}/`;
      // const paymentURL = `http://localhost:8000/api/generic-donation/subscription/${this.donationId}/`;
      try {
