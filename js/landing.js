(function () {
  ...
  $.getJSON('https://api.djnd.si/djndLanding/projects/3/')
    .done(function (json) {
      if (json.status != 'OK') {
        return;
      }
  ...
  $.getJSON('https://agrument.danesjenovdan.si/get/last-agrument/')
    .done(function (json) {
      var post = json.post;
      ...
      var image = 'https://agrument.danesjenovdan.si/media/' + post.imageURL;
      var tile = templates.makeAgrumentTile(post.title, post.content, dateString, image, post.imageCaption, post.imageCaptionURL);
      container.append(tile);
  ...
  $.getJSON('https://api.djnd.si/djndLanding/parlameter/1/')
    .done(function (data) {
  ...
  $.getJSON('https://api.djnd.si/djndLanding/news/4/')
    .done(function (json) {
      if (json.status != 'OK') {
        return;
      }
